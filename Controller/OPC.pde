...
public class OPC
{
  ...
  // Normailzed to 1/2 sized pixels so things can land in the centers between pixels
  
  void nled( int index, int x, int y) {
    led( index , x*2, y*2 ); 
  }
...
// Small Panel is 22x102 (11x51 actual LEDs with 2x spacing) 
// returns endindex

int addSmallPanel( int index, int centerx, int centery  ) {
  
    int topy = centery -(26*2);
    int bottomy = centery + (26*2);
    
    int rightx = centerx+(6*2);
    int leftx = centerx-(6*2);
    
    // top right half
    for( int x=centerx; x<rightx; x+=2 ) {
        led( index++ , x , topy);
    }
    
    // right side
    for( int y=topy+1; y<bottomy; y+=2 ) {    // +1 becuase no led in the corner
        led( index++, rightx, y );
    }
    
    //bottom
    for( int x=rightx-1; x>leftx; x-=2) {
       led( index++, x , bottomy ); 
    }
    
    //left side
    for( int y=bottomy-1; y>topy; y-=2)  {
       led( index++, leftx , y);
    }
    
    // top left half (not including middle pixel)
    for( int x=leftx+1; x< centerx; x+=2) {
       led( index++ , x , topy); 
    }
